...
import org.elasticsearch.action.support.replication.PrimaryReroute;
import org.elasticsearch.action.support.replication.ReplicaOperation;
import org.elasticsearch.action.support.replication.TransportReplicationAction2;
 ...
 * </ul>
 */
public class TransportIndexAction2 extends TransportReplicationAction2<IndexRequest, IndexRequest, IndexResponse> {
    ...
    @Inject
    public TransportIndexAction2(Settings settings, TransportService transportService, ClusterService clusterService,
    ...
    @Override
    protected IndexShardReferenceBase createShardReference(IndexShard shard, Releasable operationLock) {
        return new IndexingShardReference(shard, operationLock);
    }
    ...
    private class IndexingShardReference extends IndexShardReferenceBase {

        IndexingShardReference(IndexShard indexShard, Releasable operationLock) {
            super(indexShard, operationLock);
        }

        @Override
        public Tuple<IndexResponse, IndexRequest> shardOperationOnPrimary(MetaData metaData, IndexRequest request) throws Exception {
        ...
        @Override
        public void shardOperationOnReplica(IndexRequest request) {
     ...
     * {@link ReplicaOperation.RetryOnReplicaException} if the operation needs to be re-tried.
     */
    public static Engine.Index executeIndexRequestOnReplica(IndexRequest request, IndexShard indexShard) {
        final ShardId shardId = indexShard.shardId();
        SourceToParse sourceToParse = SourceToParse.source(SourceToParse.Origin.REPLICA, request.source()).index(shardId.getIndexName())
        ...
        if (update != null) {
            throw new ReplicaOperation.RetryOnReplicaException(shardId, "Mappings are not available on the replica yet, triggered update:" +
                    " " + update);
        }
    ...
    public static Engine.Index prepareIndexOperationOnPrimary(IndexRequest request, IndexShard indexShard) {
        SourceToParse sourceToParse = SourceToParse.source(SourceToParse.Origin.PRIMARY, request.source()).index(request.index()).type
     ...
     * {@link PrimaryReroute.RetryOnPrimaryException} if the operation needs to be re-tried.
     */
    public static WriteResult<IndexResponse> executeIndexRequestOnPrimary(IndexRequest request, IndexShard indexShard,
        ...
        if (update != null) {
            ...
            if (update != null) {
                throw new PrimaryReroute.RetryOnPrimaryException(shardId,
        ...
        return new WriteResult<>(new IndexResponse(shardId, request.type(), request.id(), request.version(), created), operation
                .getTranslogLocation());
    }
