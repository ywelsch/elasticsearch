...
public abstract class TransportWriteAction<
    ...
    /**
     * Executes the given indexing request on a replica shard, throwing a
     * {@link RetryOnReplicaException} if the operation needs to be re-tried.
     */
    protected static Engine.IndexResult executeIndexOperationOnReplica(long seqNo, long primaryTerm, long version, VersionType versionType,
                                                                       long autoGeneratedTimeStamp, boolean isRetry,
                                                                       SourceToParse sourceToParse, IndexShard replica) throws IOException {

        final Engine.Index operation;
        try {
            operation = prepareIndexOperationOnReplica(seqNo, primaryTerm, version, versionType, autoGeneratedTimeStamp, isRetry,
    ...
    public static Engine.Index prepareIndexOperationOnReplica(long seqNo, long primaryTerm, long version, VersionType versionType,
    ...
    /**
     * Executes the given deletion request on a replica shard, throwing a
     * {@link RetryOnReplicaException} if the operation needs to be re-tried.
     */
    protected static Engine.DeleteResult executeDeleteOperationOnReplica(long seqNo, long primaryTerm, long version, String type, String id,
                                                                         VersionType versionType, IndexShard replica)
        throws Exception {
        if (replica.indexSettings().isSingleType()) {
            // We need to wait for the replica to have the mappings
            Mapping update;
            try {
                update = replica.mapperService().documentMapperWithAutoCreate(type).getMapping();
        ...
        final Engine.Delete delete = replica.prepareDeleteOnReplica(type, id, seqNo, primaryTerm, version, versionType);
        return replica.delete(delete);
    }

    protected static Engine.NoOpResult executeNoOpOnReplica(long seqNo, long primaryTerm, String reason,
                                                            IndexShard replica) throws IOException {
        final Engine.NoOp noOp = replica.prepareMarkingSeqNoAsNoOpOnReplica(seqNo, primaryTerm, reason);
