...
import static org.elasticsearch.action.support.master.TransportMasterNodeActionUtils.runMasterOperation;
...
public class ClusterStateChanges {
    ...
    public ClusterStateChanges() {
        ...
        RandomAllocationDeciderTests.RandomAllocationDecider randomAllocationDecider =
        ...
        allocationService = new AllocationService(settings, new AllocationDeciders(settings,
            new HashSet<>(Arrays.asList(new SameShardAllocationDecider(settings),
                ...
                randomAllocationDecider))), NoopGatewayAllocator.INSTANCE, new BalancedShardsAllocator(settings),
        ...
        // createIndexService creates indices on the indicesService to check well-formedness -> fake it here
        try {
    ...
    public ClusterState createIndex(ClusterState state, CreateIndexRequest request) {
        return executeTask(state, () -> runMasterOperation(transportCreateIndexAction, request, state, emptyListener()));
    }

    public ClusterState closeIndices(ClusterState state, CloseIndexRequest request) {
        return executeTask(state, () -> runMasterOperation(transportCloseIndexAction, request, state, emptyListener()));
    }

    public ClusterState openIndices(ClusterState state, OpenIndexRequest request) {
        return executeTask(state, () -> runMasterOperation(transportOpenIndexAction, request, state, emptyListener()));
    }

    public ClusterState deleteIndices(ClusterState state, DeleteIndexRequest request) {
        return executeTask(state, () -> runMasterOperation(transportDeleteIndexAction, request, state, emptyListener()));
    }

    public ClusterState updateSettings(ClusterState state, UpdateSettingsRequest request) {
        return executeTask(state, () -> runMasterOperation(transportUpdateSettingsAction, request, state, emptyListener()));
    }

    public ClusterState reroute(ClusterState state, ClusterRerouteRequest request) {
        return executeTask(state, () -> runMasterOperation(transportClusterRerouteAction, request, state, emptyListener()));
    }
    ...
    private ClusterState executeTask(ClusterState state, Runnable runnable) {
    ...
    private static <T> ActionListener<T> emptyListener() {
