...
public class FullRollingRestartIT extends ESIntegTestCase {
    ...
    public void testNoRebalanceOnRollingRestart() throws Exception {
        ...
        for (RecoveryState recoveryState : recoveryResponse.shardRecoveryStates().get("test")) {
            assertTrue("relocated from: " + recoveryState.getSourceNode() + " to: " + recoveryState.getTargetNode() + "\n" + state.prettyPrint(), recoveryState.getType() != RecoverySource.PRIMARY || recoveryState.getPrimary() == false);
        }
        ...
        for (RecoveryState recoveryState : recoveryResponse.shardRecoveryStates().get("test")) {
           assertTrue("relocated from: " + recoveryState.getSourceNode() + " to: " + recoveryState.getTargetNode()+ "-- \nbefore: \n" + state.prettyPrint() + "\nafter: \n" + afterState.prettyPrint(), recoveryState.getType() != RecoverySource.PRIMARY || recoveryState.getPrimary() == false);
        }
    }
}
