...
public class RecoverySourceTests extends ESTestCase {
    ...
    public void testRecoverySourceTypeOrder() {
        assertEquals(RecoverySource.Type.EMPTY_STORE.id(), (byte) 0);
        assertEquals(RecoverySource.Type.EXISTING_STORE.id(), (byte) 1);
        assertEquals(RecoverySource.Type.PEER.id(), (byte) 2);
        assertEquals(RecoverySource.Type.SNAPSHOT.id(), (byte) 3);
        assertEquals(RecoverySource.Type.LOCAL_SHARDS.id(), (byte) 4);
        // check exhaustiveness
        for (RecoverySource.Type type : RecoverySource.Type.values()) {
            assertThat(type.id(), greaterThanOrEqualTo((byte) 0));
            assertThat(type.id(), lessThanOrEqualTo((byte) 4));
        }
    }
}
