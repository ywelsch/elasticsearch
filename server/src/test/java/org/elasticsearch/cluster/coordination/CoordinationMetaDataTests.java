...
public class CoordinationMetaDataTests extends ESTestCase {
    ...
    public void testCoordinationMetaDataSerializationEqualsHashCode() {
        CoordinationMetaData initialMetaData = new CoordinationMetaData(randomNonNegativeLong(), randomVotingConfig(), randomVotingConfig(),
            0, Sets.newHashSet(generateRandomStringArray(randomInt(10), 20, false)), randomDiscoveryNodeSet());
            ...
            meta -> {
                ...
                switch (randomInt(5)) {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        int maxMasterNodesFailureTolerance =
                            Math.max((Math.min(
                                meta.getLastCommittedConfiguration().getNodeIds().size(),
                                meta.getLastAcceptedConfiguration().getNodeIds().size()) / 2) - 1, 0);
                        int newMNFT = randomInt(maxMasterNodesFailureTolerance);
                        if (newMNFT != meta.masterNodesFailureTolerance()) {
                            builder.masterNodesFailureTolerance(maxMasterNodesFailureTolerance);
                        } else {
                            builder.term(randomValueOtherThan(meta.term(), ESTestCase::randomNonNegativeLong));
                        }
                        break;
                    case 4:
                        builder.retiredNodes(randomValueOtherThan(meta.retiredNodes(),
